...
describe("HTTP API (e2e)", () => {
  ...
  let deletedUserAuthHeader: string;
  ...
  beforeAll(async () => {
    ...
    deletedUserAuthHeader = `Bearer ${moduleFixture.get(JwtService).sign({ sub: "deleted-user" })}`;
  });

  beforeEach(() => {
    jest.resetAllMocks();

    appServiceMock.getHealthCheck.mockReturnValue(
      "App up and running on http://localhost/3003",
    );
    ...
    userServiceMock.getProfile.mockImplementation((userId) => {
    ...
    userServiceMock.updateLocation.mockImplementation((userId, location) => {
      if (userId === "deleted-user") {
        throw new NotFoundException("User not found");
      }
      return Promise.resolve({
    ...
    userServiceMock.changeAuthenticatedPassword.mockImplementation(
      (_userId, dto) => {
        if (_userId === "deleted-user") {
          throw new NotFoundException("User not found");
        }
        if (dto.currentPassword === "wrong-password") {
          throw new UnauthorizedException("Current password is invalid");
        }
        return Promise.resolve();
      },
    );
    userServiceMock.createUser.mockImplementation((dto) =>
      Promise.resolve({ ...userFixture, ...dto }),
    );
    userServiceMock.deleteUser.mockImplementation(({ id }) =>
    ...
    userServiceMock.changePassword.mockImplementation(
      (resetToken, password) => {
    ...
    cardServiceMock.createCard.mockImplementation((dto, userId) =>
      Promise.resolve({ ...cardFixture, ...dto, user: userId }),
    );
    cardServiceMock.importCards.mockImplementation((dto, userId) =>
      Promise.resolve({ imported: dto.cards.length, userId }),
    );
    cardServiceMock.deleteCard.mockImplementation((cardId) =>
      Promise.resolve({ ...cardFixture, _id: cardId }),
    );
    cardServiceMock.updateCard.mockImplementation((cardId, dto) =>
      Promise.resolve({ ...cardFixture, _id: cardId, ...dto }),
    );

    tradeServiceMock.createTrade.mockImplementation((dto, userId) =>
      Promise.resolve({ ...tradeFixture, ...dto, user: userId }),
    );
    ...
    tradeServiceMock.deleteTrade.mockImplementation((tradeId) =>
      Promise.resolve({ ...tradeFixture, _id: tradeId }),
    );
    tradeServiceMock.updateTrade.mockImplementation((tradeId, dto) =>
      Promise.resolve({ ...tradeFixture, _id: tradeId, ...dto }),
    );
    tradeServiceMock.acceptTrade.mockImplementation((userId, tradeId, dto) =>
      Promise.resolve({
        ...tradeFixture,
        _id: tradeId,
        acceptedBy: userId,
    ...
    tradeServiceMock.declineTrade.mockImplementation((userId, tradeId, dto) =>
      Promise.resolve({
        ...tradeFixture,
        _id: tradeId,
        declinedBy: userId,
    ...
    tradeServiceMock.findTradesByUser.mockImplementation((userId) =>
      Promise.resolve([{ ...tradeFixture, user: userId }]),
    );

    messageServiceMock.createMessage.mockImplementation((dto, userId) =>
      Promise.resolve({ ...messageFixture, ...dto, user: userId }),
    );
    ...
    setServiceMock.findAll.mockResolvedValue([
    ...
    caslServiceMock.checkReadForTradeById.mockImplementation(
      (trade, userId) => {
    ...
    caslServiceMock.checkCreateForMessage.mockImplementation(
      (trade, userId) => {
        if (trade.user !== userId && trade.trader !== userId) {
          throw new UnauthorizedException("You cannot access this trade");
        }
    ...
    caslServiceMock.checkReadForMessageByTrade.mockImplementation(
      (trade, userId) => {
        if (trade.user !== userId && trade.trader !== userId) {
          throw new UnauthorizedException("You cannot access this trade");
        }
    ...
    caslServiceMock.checkDeleteForMessage.mockImplementation(
      (message, userId) => {
        if (message.user !== userId) {
  ...
  it("GET /user/me returns only the authenticated user profile", async () => {
    await request(server)
      .get("/user/me")
      .set("Authorization", authHeader)
      .expect(200)
      .expect(({ body }) => {
        ...
        expect(body.isBulkImport).toBe(true);
        expect(body.requestedFor).toBe("auth-user");
        expect(body).not.toHaveProperty("password");
      });
  });

  it("GET /user/me rejects unauthenticated requests", async () => {
    await request(server).get("/user/me").expect(401);
  });

  it("GET /user/me returns 404 when the authenticated user was deleted", async () => {
    await request(server)
      .get("/user/me")
      .set("Authorization", deletedUserAuthHeader)
      .expect(404);
  });

  it("PATCH /user/me/location updates the authenticated user location", async () => {
    await request(server)
      .patch("/user/me/location")
      .set("Authorization", authHeader)
      .send({ latitude: 48.85, longitude: 2.35 })
      .expect(200)
      .expect(({ body }) => {
  ...
  it("PATCH /user/me/location validates coordinates", async () => {
    await request(server)
      .patch("/user/me/location")
      .set("Authorization", authHeader)
      .send({ latitude: 120, longitude: 2.35 })
      .expect(400);
  });

  it("PATCH /user/me/location rejects unauthenticated requests", async () => {
    await request(server)
      .patch("/user/me/location")
      .send({ latitude: 48.85, longitude: 2.35 })
      .expect(401);
  ...
  it("PATCH /user/me/location returns 404 when the authenticated user was deleted", async () => {
    await request(server)
      .patch("/user/me/location")
      .set("Authorization", deletedUserAuthHeader)
      .send({ latitude: 48.85, longitude: 2.35 })
  ...
  it("PATCH /user/me/password changes the authenticated user password", async () => {
    await request(server)
  ...
  it("PATCH /user/me/password accepts a legacy short current password", async () => {
    await request(server)
      .patch("/user/me/password")
      .set("Authorization", authHeader)
      .send({ currentPassword: "short", newPassword: "new-password" })
      .expect(204);
  });

  it("PATCH /user/me/password validates password length", async () => {
    await request(server)
      .patch("/user/me/password")
      .set("Authorization", authHeader)
  ...
  it("PATCH /user/me/password rejects an incorrect current password", async () => {
    await request(server)
      .patch("/user/me/password")
      .set("Authorization", authHeader)
  ...
  it("PATCH /user/me/password rejects unauthenticated requests", async () => {
    await request(server)
      .patch("/user/me/password")
      .send({ currentPassword: "old-password", newPassword: "new-password" })
      .expect(401);
  ...
  it("PATCH /user/me/password returns 404 when the authenticated user was deleted", async () => {
    await request(server)
      .patch("/user/me/password")
      .set("Authorization", deletedUserAuthHeader)
      .send({ currentPassword: "old-password", newPassword: "new-password" })
  ...
  it("DELETE /user validates the delete payload", async () => {
    await request(server).delete("/user").send({}).expect(400);
  });
  ...
  it("GET /user/verify/:verify returns 404 for an unknown token", async () => {
    await request(server).get("/user/verify/missing-token").expect(404);
  });
  ...
  it("POST /card/import rejects unauthenticated requests", async () => {
    await request(server).post("/card/import").send({ cards: [] }).expect(401);
  });
  ...
  it("GET /foil returns all foils", async () => {
    await request(server).get("/foil").expect(200).expect(["foil", "nonfoil"]);
  });
});
