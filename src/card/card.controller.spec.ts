...
describe("CardController", () => {
  ...
  it("should call card service createCard()", () => {
    ...
    expect(cardServiceProviderMock.createCard).toHaveBeenLastCalledWith(
      createCardDto,
      reqMock.user.userId,
    );
  });
  ...
  it("should call card service importCards()", () => {
    ...
    expect(cardServiceProviderMock.importCards).toHaveBeenLastCalledWith(
      importDto,
      reqMock.user.userId,
    );
  });
  ...
  it("should call card service deleteCard()", async () => {
    ...
    expect(caslServiceMock.checkForCard).toHaveBeenCalledWith(
      cardMock,
      userId,
      Action.Delete,
    );
  ...
  it("should call card service findCardByUser", () => {
    ...
    // When
    controller.findByUser(userId, { limit: "6", order: SortOrder.DESC });

    // Then
    expect(cardServiceProviderMock.findCardByUser).toHaveBeenCalledTimes(1);
    expect(cardServiceProviderMock.findCardByUser).toHaveBeenLastCalledWith(
      userId,
      { limit: "6", order: SortOrder.DESC },
    );
  });
  ...
  it("should call card service findCard", () => {
    ...
    // Then
    expect(cardServiceProviderMock.findCardById).toHaveBeenCalledTimes(1);
    expect(cardServiceProviderMock.findCardById).toHaveBeenLastCalledWith(
      cardId,
    );
  });
  ...
  it("Should call card service updateCard", async () => {
    ...
    expect(cardServiceProviderMock.findCardById).toHaveBeenCalledWith(cardId);
    expect(caslServiceMock.checkForCard).toHaveBeenCalledTimes(1);
    expect(caslServiceMock.checkForCard).toHaveBeenLastCalledWith(
      cardMock,
      userId,
      Action.Put,
    );
    expect(cardServiceProviderMock.updateCard).toHaveBeenCalledTimes(1);
    expect(cardServiceProviderMock.updateCard).toHaveBeenCalledWith(
      cardId,
      cardUpdateDto,
    );
  });
